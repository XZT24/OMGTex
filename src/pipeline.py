...
from tex_lmk_detect_model import KeypointResNet
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if gradient_guided_refine==True:
                    pred_x0 = latents-noise_pred*t/1000 # use in the future.                           
                    pred_x0 = self._unpack_latents(pred_x0, height, width, self.vae_scale_factor)
                    pred_x0 = (pred_x0 / self.vae.config.scaling_factor) + self.vae.config.shift_factor

                    image_x0 = self.vae.decode(pred_x0.to(dtype=self.vae.dtype), return_dict=False)[0]
                    model = KeypointResNet(num_keypoints=8).to(device)
                    model.load_state_dict(torch.load(lmk_detector_path, map_location=device))
                    model = model.to(torch.bfloat16)
                    model.eval()
                    outputs = model(image_x0)

                    mask = torch.zeros(1, 16, dtype=torch.int32).to(device).to(torch.bfloat16)
                    mask[0, 1::2] = 1  #

                    if i>=align_start_step and i < align_end_step:                
                        criterion = nn.MSELoss()
                        loss = criterion(outputs*mask, gt_lmk*mask) # gt_lmk refers to the template texture landmark
                        grad_E = torch.autograd.grad(loss, latents)[0]                
                        latents = latents - grad_E*align_weight
